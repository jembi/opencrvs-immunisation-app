...
module.exports = function () {
  var setField = function(resource, key, value, params){
    ...
    var arrayIndex = null;
    if (arrayIndexMatch) {
      var arrayIndex = arrayIndexMatch[1];
  ...
  var setDataInResource = function (value, resource, path, params) {
      ...
      if ( pathArray.length > 1 ){
        ...
        if (arrayIndexMatch) {
          ...
          var pathTemp = path.replace(pathRootLevel+'.', "")
          var resourceTemp = resource[pathRootLevelTemp][arrayIndex];
        }else{
          var pathTemp = path.replace(pathRootLevel+'.', "")
          var resourceTemp = resource[pathRootLevel];  
        }

        var paramsTemp = null;
        if ( params ){
          var paramsTemp = params.replace(pathRootLevel + '.', "")
        }
